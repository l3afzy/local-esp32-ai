...
> **It might make a mistake.** It was tested carefully (see [Test questions it never trained on](#test-questions-it-never-trained-on)), but no test covers every way a question can be asked. Double-check anything that matters, especially health, safety and money. It is not a substitute for a first-aid course, survival training or emergency services: **in an emergency, call for help first** (911, 112, 999 or your local number) if you can.

```
you: how do i purify water
esp: Boil it for 1 minute (3 above 6,500 ft).
you: how much bleach for 4 gallons
esp: 32 drops of 6% bleach; wait 30 min.
...
you: how do i find north at night
esp: Follow the Big Dipper's 2 end stars to Polaris.
you: how much water for 4 people for 3 days
esp: 12 gallons (45 L) for 3 days.
you: how much does a hummingbird weigh
esp: Between 2 and 20 grams.
you: who was the 16th president
esp: Abraham Lincoln.
you: 15% tip on 42.50
...
| | Facts | Where from |
|---|---|---|
| **Survival** | **197**, plus survival math | CDC, NWS, NPS, Ready.gov / FEMA, Red Cross, NHS, WHO, US Army survival manual: `data/facts_survival.tsv` |
| People, places, countries, books, films, songs, science | 21,369 | Wikidata (CC0), filtered for reliability: `data/wikidata_facts.py` |
| Elements, currencies, calling codes, constants, capitals | 2,527 | mendeleev, Unicode CLDR, phonenumbers, SciPy: `data/more_facts.py` |
| General and everyday: animals, space, history, food safety, first aid, emergency numbers | 761 | hand-written: `data/facts.tsv` |
| **Total** | **24,854 facts, 97,490 accepted phrasings** | |

Anything that can be calculated is calculated instead: math, money, units, times, dates, and survival sums (water supplies, bleach doses, lightning distance).
...
You need [PlatformIO](https://platformio.org/install/cli) and any ESP32 board with 4 MB of flash.
...
python train/train.py          # about 3 hours on a 4-core CPU, no GPU needed
make -C host                   # the PC build of the engine; export uses it
python train/export.py         # int4 + gate index + errata -> firmware/data/model.bin + firmware/src/model_data.h
python train/check_routing.py && python train/evaluate.py
```

`check_routing.py` takes seconds and needs no trained model: it checks that every phrasing and every test question reaches the right fact. Run it after any data change, before hours of training.

To teach it new facts, add lines to `data/facts.tsv` (or `data/facts_survival.tsv`) and retrain. Every phrasing is accepted by the gate; the model learns the shortest one. A phrasing starting with `~` is accepted but never used as the key, so it can be added without retraining:

```
question|another phrasing|~a phrasing added later<TAB>Direct answer.
```
...
python data/wikidata_facts.py      # queries are cached in data/wikidata_cache/
```
...
| [Test questions it never trained on](#test-questions-it-never-trained-on) | 507 / 510, and the 3 misses are `I don't know.` |
